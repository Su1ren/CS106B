...
int countZeroSumSubsets(Vector<int> &v, int index, int sumSoFar) {
    if (index == v.size()) {
        ...
        return countZeroSumSubsets(v, index + 1, sumSoFar + v[index]) // 选择该元素，和加进来
             + countZeroSumSubsets(v, index + 1, sumSoFar); // 不选择该元素，和不加
    }
}
...
int buggyCount(Vector<int> &v, int index, int sumSoFar) {
    if (index == v.size()) {
        ...
        return buggyCount(v, index + 1, sumSoFar + v[index]) // 选择该元素，但是不能是 += 会导致 sumSoFar 改变
             + buggyCount(v, index + 1, sumSoFar); // 这里的 sumSoFar 是错误的值
    }
}
