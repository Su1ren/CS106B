...
STUDENT_TEST("negative leads to infinitive loop") {
    EXPECT_ERROR(factorial(-3));
}

STUDENT_TEST("mypower(), only has 3 specific considerations") {
...
STUDENT_TEST("0 order 0 is no defined, only has a limit of 1") {
    EXPECT_NO_ERROR(myPower(0,0));
    EXPECT_EQUAL(myPower(0,0),pow(0,0));
    EXPECT_EQUAL(1,pow(0,0));
}
